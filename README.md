...
- **Text wrapping** — a box grows to fit its text only up to `maxNodeW` (default 340px, scaled by `fontSize`); past that the text wraps instead. Without a ceiling one long string sets the column width for its whole row, so a single node stretches the board — and a wide enough board hits the 4096px raster cap and is downscaled until nothing reads. Titles wrap to two lines, each `details` line to three, then ellipsize. A word too long to break is never split: it overflows and widens the box, so module paths and ids stay readable. To opt out, set `"maxNodeW": 0` for the board, or give one box an explicit `"w"` — an explicit width always wins.
